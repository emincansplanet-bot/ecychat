...
export async function GET(req: Request) {
  ...
  for (const r of rows) {
    const metaStr =
      r.meta == null
        ? ""
        : JSON.stringify(r.meta as Record<string, unknown>).replace(/\s+/g, " ").trim();
    lines.push(
      csvLine([
  ...
  const fname = `ecychat-denetim-${filenameDate(from)}_${filenameDate(to)}.csv`;
